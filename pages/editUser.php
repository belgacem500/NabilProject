...
if (!isset($_SESSION["loggedin"]) || $_SESSION["loggedin"] !== TRUE) {
    ...
    exit;
} elseif ((!isset($_GET['id']) && !isset($_POST['userid']))) {
...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    if (isset($_GET['id'])) {
        $row = $reg->getUsersById($_GET['id']);
    }
} else {
    $row = $reg->getUsersById($_POST['userid']);
}

# Processing form data when form is submitted
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    # Validate username
    if ($reg->checkUserName($_POST["username"]) != 0 && $reg->checkUserName($_POST["username"]) != $row["id"]) {
    ...
    if ($reg->checkUserEmail($_POST["email"]) != 0 && $reg->checkUserEmail($_POST["email"]) != $row["id"]) {
        #Check if there is a user with the same name
